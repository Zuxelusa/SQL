...
SELECT c.name AS 'Кличка', s.shopname AS 'Название магазина'
FROM cats as c
...
FROM shops
WHERE EXISTS 
(SELECT shops_id FROM cats WHERE cats.name = 'Murzik' AND shops.id = cats.shops_id);
...
SELECT shops.shopname AS "В них нет Мурзика и Зузы"
FROM shops
JOIN cats 
ON shops.id = cats.shops_id
...
ON o.ord_an = a.an_id
WHERE o.ord_datetime BETWEEN "20200205" AND "20200205" + 8;